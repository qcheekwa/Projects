...
AMPSNetwork::~AMPSNetwork()
{
    ...
    if(m_replyPackage!=NULL) delete m_replyPackage;
...
void AMPSNetwork::Init()
{
  ...
  m_replyPackage = new AMPSNetworkReply;
...
void AMPSNetwork::SetReplyData(AMPSNetworkReply *rdata)
{
  m_replyPackage = rdata;
...
void AMPSNetwork::postDownloadAssetRequest(QUrlQuery &params, const QByteArray* uploadFile)
{
  QString requestUrl = m_AMPSWebServicesPath + "downloadAsset";
  QUrlQuery qparam;
/*
  qparam.addQueryItem("tokenid", params
  qparam.addQueryItem("userid",
  qparam.addQueryItem("projectid",
  qparam.addQueryItem("assetid",
  qparam.addQueryItem("revid", 
*/
  ...
  m_downloadedFile = new QFile(folderDest + "/" + filename);
...
printf("\n\n");
  ...
  m_reply = m_networkManager->post(request, data);  // post request here
...
//QString resultRaw = m_reply->readAll();
    ...
    toSearch.push_back(",\"latest_revsize\":");
    ...
    for(QVector<QString>::Iterator p=toSearch.begin();p!=toSearch.end();++p)
    {
        ...
        while(resultPos>-1)
        {
            ...
            if(resultPos>-1)
            {
                ...
                {
                    int endPos1 = resultRaw.indexOf(",", resultPos+1);
                    int endPos2 = resultRaw.indexOf("}", resultPos+1);
                    int endPos;

                    if(endPos1<0 && endPos2 > 0) endPos = endPos2;
                    else if(endPos1>0 && endPos2<0) endPos = endPos1;
                    else if(endPos1<endPos2) endPos = endPos1;
                    else endPos=endPos2;
...
//    std::cout << "Error :" << n_errorMessages.toStdString() << std::endl;
    ...
    {
        m_replyPackage->errorFlags = 0; // '0' indicate no err
    }
...
//    std::cout << m_reply->bytesAvailable() << " " << m_reply->bytesToWrite() << "======";
    ...
    m_downloadedFile->flush();
    //emit(FileReadyRead(this));
    //emit(FileReadyRead(m_downloadedFile->fileName()));
